...
public class UniformMultiGenMutationTest {
    ...
    private UniformMultiGenMutation uniformMultiGenMutation;
    ...
    @Test
    public void uniformMutateOnlyHeightAndChest() {
        List<Allele> mutatedAlleles = uniformMultiGenMutation.mutate(dad, itemsProvider);
        assertEquals("Height should be mutated and be MAX_HEIGHT", 2d, ((Height) mutatedAlleles.get(0)).getValue(), 0);
        assertEquals("Boots should be the same", 1, ((Item) mutatedAlleles.get(1)).getId());
        assertEquals("Chest should have mutated", 10, ((Item) mutatedAlleles.get(2)).getId());
        assertEquals("Gloves should be the same", 1, ((Item) mutatedAlleles.get(3)).getId());
